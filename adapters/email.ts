...
async function pollExisting(imap: ImapFlow, nc: NatsConnection) {
...
async function main() {
  ...
  await pollExisting(imap, nc);

  // Open INBOX — ImapFlow auto-enters IDLE mode
  const lock = await imap.getMailboxLock("INBOX");

  // Listen for new messages (IDLE push)
  imap.on("exists", async (data: { path: string; count: number; prevCount: number }) => {
    ...
    try {
      // Fetch only new UIDs since our last known count
      const result = await imap.search({ seen: false }, { uid: true });
  ...
  // Fallback polling every 60s in case IDLE misses something
